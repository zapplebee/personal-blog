...
import { getPubDates } from "./pubDate";
...
export async function render(data, children) {
  ...
  const pubDate = getPubDates(data.page.inputPath)[0];
...
${
  process.env.NODE_ENV === "production"
    ? `
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-3KX9BK97EH"></script>
    <script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-3KX9BK97EH');
</script>`
    : ""
}
        </head>`;
